...
<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        $sql_check_email = "SELECT * FROM user_form WHERE email = '$user_email'";
        ...
        if ($result->num_rows > 0) {
            ...
            $email = $_POST['email'];
            $sexe = $_POST['sexe'];
            $situationFamiliale = $_POST['sitiationFamiliale'];
            ...
            $accessLevelOption = isset($_POST['accessLevelOption']) ? $_POST['accessLevelOption'] : "";
            $type_diplome = isset($_POST['type_diplome']) ? $_POST['type_diplome'] : "";
            $target_dir = "uploads/".$nom.".".$prenom."/";
            ...
            $cin_file = $nom . ".cin.pdf";
            $bac_file = $nom . ".bac.pdf";
            $releve = $nom . ".releve.pdf";
            $diplome_file = isset($_FILES["diplome_file"]["name"]) ? $nom . ".diplome." . pathinfo($_FILES["diplome_file"]["name"], PATHINFO_EXTENSION) : "";        
            move_uploaded_file($_FILES["image"]["tmp_name"], $target_dir . $image);
            move_uploaded_file($_FILES["cin_file"]["tmp_name"], $target_dir . $cin_file);
            move_uploaded_file($_FILES["bac_file"]["tmp_name"], $target_dir . $bac_file);
            move_uploaded_file($_FILES["releve"]["tmp_name"], $target_dir . $releve);
            if ($diplome_file !== "") {
                move_uploaded_file($_FILES["diplome_file"]["tmp_name"], $target_dir . $diplome_file);
            }
            $sql = "INSERT INTO user_form (`optionn`, cin, pays, `user-name`, email, nom, prenom, arb_nom, arb_prenom, `password`, sexe, sitiationFamiliale, dateNaissance, adresse, ville, tel, diploma, accessLevel, accessLevelOption,  type_diplome, `image`, cin_file, bac_file, releve, diplome_file)
            VALUES ('$option', '$cin','$pays', '$user_name','$email', '$nom', '$prenom', '$arb_nom', '$arb_prenom', '$password', '$sexe', '$situationFamiliale', '$dateDeNaissance', '$adresse', '$ville', '$tel', '$diploma', '$accessLevel', '$accessLevelOption', '$type_diplome', '$image', '$cin_file', '$bac_file', '$releve', '$diplome_file')";
        ...
        if (!empty($_FILES)) {
            // Récupérer la liste des dossiers dans le répertoire "uploads"
            $folders = glob('uploads/*', GLOB_ONLYDIR);
            // Parcourir chaque dossier
            foreach ($folders as $folder) {
                // Extraire le nom et le prénom de chaque dossier
                $folderName = basename($folder);
                list($nom, $prenom) = explode('.', $folderName);
                // Vérifier si le nom et le prénom correspondent à ceux du dossier
...
<form id="myForm" method="post" action="users-create.php" enctype="multipart/form-data">
        <div class="tab" id="tab-1">
        <h3>Identification</h3>
        <div class="col-6">           
            <label>User name:<span>*</span></label>
            <input type="text" name="user-name" id="userInput"  name="user-name" class="form-control" placeholder="Enter your username">
            <div id="userError" class="error-message"></div></div> <br>
            <div class="row">
            <div class="col">
            <label >Nom:<span>*</span></label>
            <input type="text" id="nameInput" name="nom" class="form-control" placeholder="Enter your name" >
            <div id="nameError" class="error-message"></div></div><br>
            <div class="col">          
            <label>Prénom:<span>*</span></label>
            <input type="text" id="prenomInput" name="prenom" class="form-control" placeholder="Enter your firstname" ><div id="prenomError" class="error-message"></div></div></div><br>
            <div class="row">
            <div class="col">
            <label> :الاسم العائلي <span>*</span></label><br>
            <input type="text" id="name_arb" class="form-control" name="arb_nom" placeholder="أدخل اسمك العائلي" ><div id="arbNomError" class="error-message"></div></div>
            <div class="col">
            <label>:الاسم الشخصي<span>*</span></label><br>
            <input type="text" id="prenom_arb" name="arb_prenom" class="form-control" placeholder="أدخل اسمك الشخصي" ><div id="arbPrenomError" class="error-message"></div></div></div>
            <div class="row">
            <div class="col">
            ...
            <input type="text" id="email" name="email" class="form-control" ><div id="emailError" class="error-message"></div></div>
            <div class="col">
            <label >CIN:</label><br>
            ...
            <input type="tel" id="tel" name="tel" class="form-control" placeholder="Entréz votre numero de telephone" >
            ...
            <div class="tab" id = "tab-2"><br><br><br>
            <h3>Formation</h3>
            <div class="col-6">
...
<script>
  document.getElementById("myForm").addEventListener("submit", function(event) {
    ...
    if (formElements.tel.value === "") {
        ...
        isValid = false;
    } else if (!/^0[0-9]{9}/.test(formElements.tel.value)) {
